...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    for epoch in range(epochs):
        ...
        for image, label in get_batches_fn(batch_size):
            ...
            loss_log.append(float('{:3f}'.format(loss)))
        print(loss_log)
        loss_epoch.append(sum(loss_log)/float(len(loss_log)))
        print()
...
def run():
    ...
    with tf.Session() as sess:
        ...
        loss_epoch = train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
                 correct_label, keep_prob, learning_rate)

        loss_dir = './loss'
        print(datetime.datetime.now().strftime('%Y%m%d%H%M%S'))
        filename = loss_dir + '/' + datetime.datetime.now().strftime('%Y%m%d%H%M%S') + '.log'
